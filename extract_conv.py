...
def main(limit=20, x_limit=3, y_limit=6):
    ...
    print('extract lines')
    fp = open("E:\VSCode\Code\Python\Chatbot\dgk_shooter_min.conv", 'r', errors='ignore', encoding='utf-8')
    groups = []
    group = []
    ...
    for group in tqdm(groups):
        for i in range(1,(int)(len(group) / 2)):
            x_data.append(group[i*2])
            y_data.append(group[i*2+1])
            '''
            last_line = None
            if i > 0:
                last_line = group[i - 1]
    ...
    pickle.dump(
        (x_data, y_data),
        open('E:\VSCode\Code\Python\Chatbot\Chatbot_1\Chatbot_2\chatbot.pkl', 'wb')
    )
    pickle.dump(ws_input, open('E:\VSCode\Code\Python\Chatbot\Chatbot_1\Chatbot_2\ws.pkl', 'wb'))
